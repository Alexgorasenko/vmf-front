...
const NodeItem = ({subject, nodeData, nodekey, changeNodes, closeNode, removeNode, addNode, lostTourns, lostLeagues}) => {
    ...
    const [type, setType] = useState(nodeData.type ? types.find(t => t.id === nodeData.type) : nodekey.length ? types.find(t => t.id === 'tournament') : types[0])
    ...
    const dialogBody = () => {
        ...
        switch(type.id) {
            ...
            case 'group':
                return  <div className='create-dialog_body'>
                            <div className='control'>
                                <InputText placeholder='Название группы' value={form.label || ''} onChange={e => setForm({...form, label: e.target.value})} />
                            </div>

                            {unassigned && unassigned.length ? <Message severity="warn" text={`возможные турниры: ${unassigned.join(', ')}`} /> : null}
    ...
    return (
        form ? <div className={'nodeItem'} style={{paddingLeft: '20px', overflow: 'scroll'}}>
            <Dialog
                visible={form}
